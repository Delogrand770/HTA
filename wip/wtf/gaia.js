/*
Created by Gavin Delphia 2007 - 2010
...
var XL = {
	Ini:function(it){
		if (it!=0){
			document.body.ondblclick = XL.Toggle;
		}
	...
	Log:function(it,it2){
		xlondata += '\r\n'+xlonsym+it;
		if (it2==1 && xlonxi.style.display == 'block'){
			XL.Update();
		}else{
			XL.Update();
		}
	...
	Export:function(){
		XS.File('xlon.txt','create',xlondata);
	...
	Update:function(){
		...
		XI.Fill('xlonxi','Console','<textarea style="background-Color:transparent;border:1 solid transparent;color:white;height:87%;width:100%;font-size:11px;scroll:auto;" readonly id="xlconout">'+xlondata+'</textarea><input type="textarea" style="background-Color:transparent;color:white;border:1 solid silver;width:100%;" onkeypress="if (event.keyCode == 13){XL.Debug(this.value);this.value=\'\';}" id="xlconin">');
...
var XS = {
	File:function(it,iact,idat){
		//if (XL.En()){XL.Log('XS.File('+it+','+iact+',data)',1)}
...
var XWS = {
	Launch:function(it){
		if (XL.En()){XL.Log('XWS.Launch('+it+')',1)}
		xshell.Run(it,0);
	},
	Reg:function(it,iact,idata){
		if (XL.En()){XL.Log('XWS.Reg('+it+','+iact+','+idata+');',1)}
		if (iact == 'read'){
	...
	SendKey:function(it){
		if (XL.En()){XL.Log('XWS.SendKey('+it+');',1)}
		xshell.SendKeys(it);
	}
};
...
var XU = {
	...
	ErrorHandler:function(m){
		if (XL.En()){XL.Log('XU.ErrorHandler('+m+') Error Handler enabled',1)}
		if (m == 0){
			window.onerror = function(msg,url,l){
				if (XL.En()){XL.Log('APP ERROR: '+msg+'\r\n\tLINE: '+l,1)}
				return true;
			}
		}
		if (m == 1){
	...
	CreateShortcut:function(it,it2,ico,des,scut){ //scut is not a param
		if (!XS.File(xshell.SpecialFolders('Desktop') + '\\' + it + '.lnk','exist')){
			scut                  = xshell.CreateShortcut(xshell.SpecialFolders('Desktop') + '\\' + it + '.lnk');
...
var nxms = new Array('silver','#0099ff',0);
var XM = {
	Ini:function(){
		if (XL.En()){XL.Log('XM.Ini() XMessage System Enabled',1)}
		document.body.innerHTML+='<div id="nxmscreen" style="height:105%;width:105%;position:absolute;top:-2;left:-2;display:none;filter:alpha(opacity=50);background-Color:white;z-index:110;"></div>';
	...
	Alert:function(ititle,ibody,itype,ilay){
		...
		if (itype == 0){
			...
			nxmbtn.style.color  = nxms[0];
			nxmbtn.onmouseover  = function(){
				nxmbtn.style.color = nxms[1];
			...
			nxmbtna.style.color  = nxms[0];
			nxmbtna.onmouseover  = function(){
				nxmbtna.style.color = nxms[1];
				nxmbtna.onmouseout  = function(){
					nxmbtna.style.color = nxms[0];
				}
			}	
			nxmbtnb.style.color  = nxms[0];
			nxmbtnb.onmouseover  = function(){
				nxmbtnb.style.color = nxms[1];
	...
	LayC:function(it){
		nxmscreen.style.backgroundColor = it;
	},
	BodyC:function(it){
...
var XI = {
	New:function(iwhere,iid,ipos,isize,idisplay,izindex,itcolor,ibcolor){ //new style
		if (XL.En()){XL.Log('XI.New('+iwhere+','+iid+')',1)}
		nxinfcanc = 0;
		...
		if (!iy || !ix){
			nxinfcanc = 1;
			XM.Alert('XInfo - '+iid,'You didnt specify <i>x:y [2]</i>',0);
		}
		if (!ih || !iw){
			nxinfcanc = 1;
			XM.Alert('XInfo - '+iid,'You didnt specify <i>h:w [3]</i>',0);
		}
		if (!iwhere){
			nxinfcanc = 1;
			XM.Alert('XInfo - '+iid,'You didnt specify <i>iwhere [0]</i>',0);
		}
		if (!iid){
			nxinfcanc = 1;
			XM.Alert('XInfo - '+iid,'You didnt specify <i>iid [1]</i>',0);
		}
		if (nxinfcanc == 0){
...
var XDS = {
	Ini:function(enc){
		if (XL.En()){XL.Log('XDS.Ini() Save Data system enabled',1)}
		xencrypt = enc;
		if (xsave_vars.length == 0){
			XM.Alert('XDataSystem - Error','You have not defined the array <font color="red">xsave_vars</font>',0);
		}
	},
	Save:function(it){
		if (XL.En()){XL.Log('XDS.Save('+it+')',1)}
		xtemp[0] = "";
		xtemp[1] = 0;
...
var XC={
	Ini:function(it1,it2,it3,it4,it5,it6,it7,it8){
		if (XL.En()){XL.Log('XC.Ini() Old Multi-connect enabled...why?',1)}
		if (it1){xconndir    = it1;}
		...
		if (xconndir==0){
			XM.Alert('XConnect','No xconndir directory specified',0);
		}else if (xdatainact==0){
			XM.Alert('XConnect','No xdatainact function specified',0);
		}else if (xdataoutact==0){
			XM.Alert('XConnect','No xdataoutact function specified',0);
		}else if (xappid==0){
			XM.Alert('XConnect','No xappid specified',0);
		}else if (xmaxplayers==0){
			XM.Alert('XConnect','No xmaxplayers specified',0);
		}else if (xminplayers==0){
			XM.Alert('XConnect','No xmminplayers specified',0);
		}else if (xgamevalid==0){
			XM.Alert('XConnect','No xgamevalid function specified',0);
		}else if (xgameready==0){
			XM.Alert('XConnect','No xgameready function specified',0);
		}
	},
	Connect:function(){
		xmyrole = 0;
		if (!XS.File(xconndir+xappid+'-host.xcon','exist')){
			...
			if (xscan.length<xmaxplayers){
				...
				XM.Alert(xappid,'Game is full',2);
			}
		}
		if (xmyrole!=0){
			if (xdev == 1){
			...
			XM.Alert(xappid,'Players connecting',2);
		}
	},
	GetOpponents:function(){
		XS.Folder(xconndir,'search','client.xcon');
		XM.Alert(xappid,'Players connecting<br><br>'+xscan.join('<br>').replace(new RegExp(xappid,"g"),"").replace(new RegExp("-client.xcon","g"),""),2);
...
var xcitimer = 0;
var xcipunam = XN.Name('user');
var XCIP={
	Ini:function(it){
		if (XL.En()){XL.Log('XCIP.Ini() New Multi-connect enabled',1)}
		if (it==1){
			XCIP.Purge();
		}
	...
	Purge:function(){
		XS.File('C:\\Users\\'+xcipunam+'\\Documents\\ipmsg.log','delete');
	},
	Clean:function(){
	...
	In:function(){
		xcipque[0] = [];
		xcipque[1] = [];
		xcipque[0] = XS.File('C:\\Users\\'+xcipunam+'\\Documents\\ipmsg.log','open');
		if (xcipque[0]){
...
var XCap = {
	Encrypt:function(idata,ipass,imode){
		if (XL.En()){XL.Log('XCap.Encrypt() called',1)}
		var temp                          = 0;
	...
	Decrypt:function(idata,ipass,imode){
		if (XL.En()){XL.Log('XCap.Decrypt() called',1)}
		var temp                          = 0;
		var EncryptedData 		  = new ActiveXObject("CAPICOM.EncryptedData");
		//EncryptedData.Algorithm.KeyLength = 0; //0-5
		//EncryptedData.Algorithm.Name      = imode; //0-4
	...
	UseCard:function(ihash){
		if (XL.En()){XL.Log('XCap.UseCard() called',1)}
		try{
