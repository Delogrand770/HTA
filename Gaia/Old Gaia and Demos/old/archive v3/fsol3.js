...
var XInfo = {
	New:function(iwhere,iid,ipos,isize,idisplay,izindex,itcolor,ibcolor){
		...
		if (!iy || !ix){
			xinfcanc = 1;
			XMessage.Alert('XInfo','You didnt specify <i>x:y [2]</i>');
		}
		if (!ih || !iw){
			xinfcanc = 1;
			XMessage.Alert('XInfo','You didnt specify <i>h:w [3]</i>');
		}
		if (!iwhere){
			xinfcanc = 1;
			XMessage.Alert('XInfo','You didnt specify <i>iwhere [0]</i>');
		}
		if (!iid){
			xinfcanc = 1;
			XMessage.Alert('XInfo','You didnt specify <i>iid [1]</i>');
		}
		if (xinfcanc == 0){
